#!/usr/bin/env python3
"""
Merges the output of sabayomikun12 and Katikati Counter.
"""
import sys
import os
...
if INTERACTMODE:
    ...
    FTYP1 = [('From sabayomikun12', '*.csv')]
    if not tkMessageBox.askokcancel('katisaba12', 'Select automatic count csv file.\n(The output file of sabayomikun12)'):
        sys.exit()
    CSVPATH1 = tkFileDialog.askopenfilename(filetypes=FTYP1, initialdir=IDIR)
    if CSVPATH1 == '':
        sys.exit()
    if not os.path.exists(CSVPATH1):
        tkMessageBox.showinfo('katisaba12', 'Missing input file.')
        sys.exit()
...
while LINESVERT < 13:
    ...
    if not INDEXLINE1[2].isdigit():
        if not uf == 'ND':
            if INTERACTMODE:
                tkMessageBox.showinfo('katisaba12', 'Input file format does not match.\nIs this the output file of sabayomikun12?')
            else:
...
if len(CONTELINE1_arr) != 12:
    if INTERACTMODE:
        tkMessageBox.showinfo('katisaba12', 'Inpt file format does not match.\nIs this the output file of sabayomikun12?')
    else:
        print('Input file format does not match.\nIs this the output file of sabayomikun12?')
    sys.exit()
if re.search('_auto.csv$', ROOTNAME1):
    if INTERACTMODE:
        tkMessageBox.showinfo('katisaba12', 'Inpt file name does not match.\nIs this the output file of sabayomikun12?')
    else:
        print('Input file format does not match.\nIs this the output file of sabayomikun12?')
    sys.exit()
...
if INTERACTMODE:
    ...
    if not tkMessageBox.askokcancel('katisaba12', 'Select manual count csv file.\n(The output file of Katikati Counter)'):
        sys.exit()
...
if 'Red' not in INDEXLINE2 or 'Green' not in INDEXLINE2 or 'Blue' not in INDEXLINE2:
    if INTERACTMODE:
        tkMessageBox.showinfo('katisaba12', 'Index line is modified, or this is not\nthe output file of Katikati Counter.')
    else:
...
if len(CONTELINE2_arr) != 12:
    if INTERACTMODE:
        tkMessageBox.showinfo('katisaba12', 'This is not the correct output file of\nKatikati Counter, or you included\nnon-graphic files.')
    else:
...
for lines in CONTELINE2:
    if not line[1].isdigit() or not line[2].isdigit() or not line[3].isdigit():
        if INTERACTMODE:
            tkMessageBox.showinfo('katisaba12', 'This is not the output file of\nKatikati Counter.')
        else:
...
if INTERACTMODE:
    tkMessageBox.showinfo('katisaba12', 'Task finished.')
