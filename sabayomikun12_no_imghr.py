...
if os.name == 'nt':
    try:
        ...
        tkMessageBox.showinfo('sabayomikun12', 'Font \"arial.ttf\" is not installed.')
        sys.exit()
else:
    try:
        PICKFONT = ImageFont.truetype("Arial.ttf", 16)
    except OSError:
        print('sabayomikun12', 'Font \"Arial.ttf\" is not installed.\napt install ttf-mscorefonts-installer')
        sys.exit()
...
if INTERACTMODE:
    ...
    if not tkMessageBox.askokcancel('sabayomikun12', 'Select input directory.'):
        sys.exit()
    INPUTPATH = tkFileDialog.askdirectory(initialdir = IDIR)
    if INPUTPATH == '':
        sys.exit()
    INPUTPATH = INPUTPATH + '/'
    if not tkMessageBox.askokcancel('sabayomikun12', 'Input directory is \n' + INPUTPATH + '\n\nSelect output Directory.'):
        sys.exit()
...
if len(filepathsarray) == 0:
    if INTERACTMODE:
        tkMessageBox.showinfo('sabayomikun12', 'No image.')
    else:
        print('No image.')
    sys.exit()
for j in filepathsarray:
    ...
    heightpanel, widthpanel = int(heightorig / 3), int(widthorig / 4)
    minradiusval, maxradiusval = int(widthpanel * 0.39), int(widthpanel * 0.58)
    horizontalarr = [['1', [0, widthpanel + marginval]], ['2', [widthpanel - marginval, widthpanel * 2 + marginval]], ['3', [widthpanel * 2 - marginval, widthpanel * 3 + marginval]], ['4', [widthpanel * 3 - marginval, widthorig]]]
    verticalarr = [['A', [0, heightpanel + marginval]], ['B', [heightpanel - marginval, heightpanel * 2 + marginval]], ['C', [heightpanel * 2 - marginval, heightorig]]]
    clippingarr = []
    ...
    for h in clippingarr:
        ...
        if not type(circles) == np.ndarray:
            ...
            if not CROPMODE:
                ...
                csvout.close()
if INTERACTMODE:
    tkMessageBox.showinfo('sabayomikun12', 'Task finished.')
